    ...
    public static class visuals
    {
        ...
        private static void MainLoop()
        {
            ...
            while (isRunning)
            {
                ...
                Vector3 localPos = (localObj.address != 0 && (localObj.Head.IsValid || localObj.HumanoidRootPart.IsValid))
                    ? GetPos(localObj.Head.IsValid ? localObj.Head : localObj.HumanoidRootPart, cache)
                    : new Vector3();
                ...
                foreach (var p in guys)
                {
                  try
                  {
                    ...
                    {
                        // PF: use all collected bones for bounding box
                        foreach (var bone in p.Bones)
                        {
                            if (!bone.IsValid) continue;
                            var w3d = GetPos(bone, cache);
                            if (w3d.x == 0 && w3d.y == 0 && w3d.z == 0) continue;

                            if (p.Head.IsValid && bone.Address == p.Head.Address) w3d.y += 0.5f;
                            ...
                            if (s2d.x != -1)
                            {
                                ...
                                valid = true;
                                if (p.Head.IsValid && bone.Address == p.Head.Address) head2d = s2d;
                            }
                        }
                        // Also include HRP if not already in bones
                        if (p.HumanoidRootPart.IsValid)
                        {
                            var hrpW = GetPos(p.HumanoidRootPart, cache);
                            var hrpS = WorldToScreenHelper.WorldToScreen(hrpW, view, res, wp);
                            if (hrpS.x != -1)
                            {
                                x1 = Math.Min(x1, hrpS.x); x2 = Math.Max(x2, hrpS.x);
                                y1 = Math.Min(y1, hrpS.y); y2 = Math.Max(y2, hrpS.y);
                                valid = true;
                            }
                        }
                    }
                    ...
                    {
                        // Standard: use specific limb fields
                    ...
                    var hrpPart = p.HumanoidRootPart.IsValid ? p.HumanoidRootPart : (p.Head.IsValid ? p.Head : new SDKInstance(0));
                    ...
                    if (p.IsPF && !string.IsNullOrEmpty(p.ToolName))
                        next.texts.Add(new Text { t = p.ToolName, p = new Point(b.Left + b.Width / 2, b.Bottom + 14), c = Colors.Yellow, s = Settings.Visuals.NameSize, ctr = true });
                    ...
                    if (Settings.Visuals.Skeleton)
                    {
                        ...
                        if (p.IsPF)
                        {
                            // PF: no named limbs — connect Head→HRP as spine,
                            // then HRP→each non-head bone to show all limb extensions
                            if (p.Head.IsValid && p.HumanoidRootPart.IsValid)
                                Join(GetPos(p.Head, cache), GetPos(p.HumanoidRootPart, cache));

                            if (p.HumanoidRootPart.IsValid && p.Bones != null)
                            {
                                var hrp3 = GetPos(p.HumanoidRootPart, cache);
                                foreach (var bone in p.Bones)
                                {
                                    try
                                    {
        ...
        public static Vector3 GetPos(SDKInstance i, Dictionary<long, long> c)
        {
            ...
            catch { c.Remove(i.Address); return new Vector3(); }
        }
    }
}