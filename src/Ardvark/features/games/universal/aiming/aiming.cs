...
namespace FoulzExternal.games.universal.aiming
{
    public static class aiming
    {
        ...
        {
            // PF: only Head and HRP are available, no named limbs
            if (p.IsPF)
            {
                SDKInstance pfPart;
                if (bone == 0 && p.Head.IsValid)
                    pfPart = p.Head;
                else if (p.HumanoidRootPart.IsValid)
                    pfPart = p.HumanoidRootPart;
                else if (p.Head.IsValid)
                    pfPart = p.Head;
                else
                    pfPart = new SDKInstance(0);
                return get_xyz(pfPart, cache);
            }
        ...
        private static RobloxPlayer find_victim(Aiming s, FoulzExternal.SDK.worldtoscreen.WorldToScreenHelper.POINT m, Dictionary<long, long> cache)
        {
            ...
            foreach (var p in targets)
            {
                ...
                try
                {
                    ...
                    // For PF, get local position from cached local player object
                    if (p.IsPF)
                    {
                        var localObj = targets.FirstOrDefault(x => x.address == lp.Address);
                        if (localObj.address != 0 && localObj.HumanoidRootPart.IsValid)
                        {
                            long lptr = SDKInstance.Mem.ReadPtr(localObj.HumanoidRootPart.Address + Offsets.BasePart.Primitive);
