        ...
        private void pop(string title, string msg, int time)
        {
            ...
            box.Loaded += (s, e) =>
            {
                try
                {
                    ...
                    ((TranslateTransform)box.RenderTransform).BeginAnimation(TranslateTransform.XProperty, new DoubleAnimation(off, 0, new Duration(TimeSpan.FromMilliseconds(350))) { EasingFunction = new CubicEase { EasingMode = EasingMode.EaseOut } });
